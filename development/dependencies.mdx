---
title: "How Omniform Manages yt-dlp and ffmpeg"
...
The binaries are stored inside the application's data directory, under a `bin/` subdirectory. The path is resolved at runtime using Tauri's `app_data_dir()` API (from the `dirs` crate), which follows each platform's standard conventions:
